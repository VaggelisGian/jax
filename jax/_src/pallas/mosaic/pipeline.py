...
def _emit_pipeline(
  ...
  in_specs = _normalize_specs(in_specs)
  out_specs = _normalize_specs(out_specs)
  get_buffer_count = lambda spec: (
      spec.pipeline_mode.buffer_count if spec.pipeline_mode is not None else 2
  )
  ...
  def wrapped(*args, allocations=None, **kwargs):
    ...
    if allocations is not None:
      ...
      return _emit_pipeline(
          ...
          num_cores=num_cores,
          core_id=core_id,
      )(*args, allocations=allocations, **kwargs)
    ...
    in_specs_flat, in_tree = tree_util.tree_flatten(in_specs_)
    out_specs_flat, out_tree = tree_util.tree_flatten(out_specs_)
    ...
    _, in_tree = tracing_registry.flatten(
        tuple(in_refs_flat), is_transformed_ref
    )
    _, out_tree = tracing_registry.flatten(
        tuple(out_refs_flat), is_transformed_ref
    )
    ...
    _, refs_tree = tracing_registry.flatten(args, is_transformed_ref)
